...
#  my $ret=system('qpdf', $file, '--stream-data=compress', '--object-streams=generate', '--decode-level=specialized', '--compression-level=9', '--linearize',$tempfile);
  ...
  for my $object (@objects2){
    ...
    if(($discard_meta ||($count>5)) && index($dict, '/Metadata ')!=-1){ #Due to how qpdf lays out linearised files, the main metadata will always be in the first few objects.
      ...
      if($t =~ m/(\/Metadata \d+ 0 R)/){;
        #print("    Dissociating $1 from $object \n");
  ...
  if($test){
    ...
    unlink($tempfile2);
    print("      Comparison failed! Something went wrong that appears to have corrupted the PDF, so the original has not been overwritten.\n");
  }
}
...
#  print("Processing object:\n$object\n");
  ...
  if($streamlen<=10240){ #But only small things.
...
#    print(" Trying cache: $hash\n");
  ...
  if($smallobjcache{$hash}){
    ...
    if($filtertype==1){
      system('minuimus_def_helper', $tempname);
    }
    $newlen = -s $tempname;
...
sub pdfcompare(){
  ...
  my $hashb=`pdftoppm "$filea" -q | sha256sum`;
...
sub processvideo(){
 ...
 testcommand('ffmpeg');
  ...
  push(@args,  '-crf', '28'); #Default is 32, but going for a bit higher quality here.
  ...
  push(@args, '-vf', 'hqdn3d=0:0:2:2,nlmeans=s=1'); #A mild denoiser.
...
sub leanify($){
  ...
  my $ret=system('/usr/bin/leanify', '-q', '--jpeg-keep-all', '--keep-icc', '--keep-exif', $file);
